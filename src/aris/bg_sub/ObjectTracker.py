...
class ObjectTracker:
    def __init__(
        self, radius: int, frames: [BgFrame], frame_threshold: int, history: int
    ):
        super().__init__()
    ...
    def __track_and_return_updated_frame(
        self, base_frames: [], updating_frame: BgFrame
    ):
        updating_objects = updating_frame.get_all_objects()
        updated_frame = BgFrame.clone_bgFrame_metadata(updating_frame)
        updated_frame = self.__track_updating_objects(
            base_frames, updating_objects, updated_frame
        )
        return updated_frame

    def __track_updating_objects(
        self, base_frames: [], updating_objects: [], updated_frame: BgFrame
    ):
        for updating_object in updating_objects:
            updated_frame = self.__compare_base_frames(
                base_frames, updating_object, updated_frame
            )
        return updated_frame

    def __compare_base_frames(
        self, base_frames: [], updating_object: BgObject, updated_frame: BgFrame
    ):
        is_same = False
        for base_frame in base_frames:
            base_objects = base_frame.get_all_objects()
            updated_frame, is_same = self.__compare_base_objects(
                base_objects, updating_object, updated_frame
            )
            if is_same:
                break
        if not is_same:
    ...
    def __compare_base_objects(
        self, base_objects: [], updating_object: BgObject, updated_frame: BgFrame
    ):
        is_same = False
        for base_object in base_objects:
            is_same, updated_frame = self.__compare_objects_and_update_frame(
                base_object, updating_object, updated_frame
            )
            if is_same:
                break
        return updated_frame, is_same

    def __compare_objects_and_update_frame(
        self, base_object: BgObject, updating_object: BgObject, updated_frame: BgFrame
    ):
        if self.__is_same(
            base_object.get_xywh(), updating_object.get_xywh(), self.radius
        ):
            updated_frame.create_and_add_object(
