...
class BBoxManager:
    ...
    def next_frame(self):
        ...
        if self.__state["current_index"] >= len(
            self.__state["frames_data"]["metadata"]
        ):
    ...
    def prev_frame(self):
        ...
        if self.__state["current_index"] < 0:
            self.__state["current_index"] = (
                len(self.__state["frames_data"]["metadata"]) - 1
            )
        self.__process_frame()
        self.__notify_observers()
    ...
    def __draw_bounding_boxes(
        self, frame, bounding_boxes, color=(0, 255, 0), thickness=1, label_start=0
    ):
        for i in range(len(bounding_boxes)):
            ...
            frame = self.__draw_label(frame, str(label_start + i), bbox, color=color)
    ...
    def __get_bounding_boxes(self):
        ...
        bounding_boxes = self.__state["frames_data"]["metadata"][current_index][
            "bounding_boxes"
        ]
...
class LeftFrame:
    def __init__(self):
        ...
        self.root = tk.Frame()
        self.image_frame = tk.Frame(master=self.root)
        self.create_canvas()
    ...
    def add_image(self, img):
        ...
        self.canvas.create_image(20, 20, anchor=tk.NW, image=self.photo_image)
...
class RightFrame:
    ...
    def __create_input_box_for_removing_label(self, master):
        label = tk.Label(master=master, text="Enter label number to operate:")
    ...
    def __mark_noise_action(self):
        label = self.__remove_label_entry.get()
        self.__bbox_manager.mark_bounding_boxes(int(label), type=BBoxType.NOISE)
