...
def extract_frames(source_file, output_file, frame_start, frame_end, skip, fps, invert):
    aris_data, _ = pyARIS.DataImport(source_file)
    frame_start, frame_end = sanitize_frame_boundaries(
        frame_start, frame_end, aris_data
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "source_file", nargs=1, type=str, help="path of the source ARIS file"
    )
    parser.add_argument(
        "output_file", nargs=1, type=str, help="path of the output video"
    )
    ...
    parser.add_argument(
        ...
        nargs=1,
        type=int,
        help="skip frames for each exported frame",
        default=0,
    )
    parser.add_argument(
        "--fps", nargs=1, type=int, help="frames per second", default=24
    )
    parser.add_argument(
        "--invert", nargs=1, type=bool, help="invert frame color", default=False
    )
