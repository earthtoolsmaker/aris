...
def main():
    ...
    (objLabel, bboxData) = label_objects(
        bgSub_frames, exportData=True, export_img=False
    )
...
def convert_bboxData_to_bgFrames(bboxData: BBoxData):
    ...
    for i in range(len(stats)):
        stat = stats[i]
        bgFrame = BgFrame.of(stat, "{}.png".format(i), bboxData.width, bboxData.height)
...
def get_annotated_frames_from_coco(from_memory=False, frames=None, export_video=False):
    ...
    if from_memory:
        annotated_images = coco_api.get_all_annotated_imgs_from_memory(
            frames, show_label=True
        )
    else:
        annotated_images = coco_api.get_all_annotated_imgs(show_label=True)
    if export_video:
        BgUtility.export_video(annotated_images, "tracked_test.mp4", invert_color=False)
...
def save_frames_as_images(frames, prefix=""):
    ...
    for i in range(len(frames)):
        frame = frames[i]
        BgUtility.save_frame_as_image(frame, path, "{}.png".format(prefix + str(i)))
